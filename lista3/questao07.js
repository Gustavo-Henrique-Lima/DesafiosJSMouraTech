...
function estatistica_naturalidade(array)
{
    ...
    array.forEach(aluno => 
    {
        cidades.push(aluno.naturalidade)    
    });
    const cidadesSemRepeticao = [...new Set(cidades)];
    const citys=[];
    cidadesSemRepeticao.forEach(cidades =>
    {
        citys.push(cidades)
    });
    let n = array.length;
    ...
    for (let i=0; i<q; i++)
    {
        for (let j=0; j < n; j++)
        {
            if (citys[i] == alunos[j].naturalidade) 
            {
                cont++;
            }
        }
    ...
    const res=arrayIntersect(citys,results);
    return res;
}
